...
app.post('/api/auth/login', (req, res) => {
  ...
  if (isOwner) {
    users[em] = { pbId: '2081902760', email: SUPER_OWNER, username: username || '☆ Lucky Ak47 🖥️☆', coins: 1000000000, role: 'owner', level: 99 };
  } else if (!users[em]) {
    users[em] = { pbId: Math.floor(10000000 + Math.random() * 90000000).toString(), email: em, username: username || em.split('@')[0], coins: 50000, role: 'member', level: 1 };
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('take_mic', (d) => {
    ...
    if (r) {
      ...
      r.seats[d.seatIndex] = { name: d.username, email: d.email, isMuted: false };
...
server.listen(PORT, '0.0.0.0', () => console.log('PB Master Live on ' + PORT));
