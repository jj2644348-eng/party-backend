...
let gifts = [
  ...
  { id: 'g3', name: '🏎️ PB Super Car', price: 1000, icon: '🏎️' },
  { id: 'g4', name: '👑 PB Crown', price: 5000, icon: '👑' },
  { id: 'g5', name: '🚀 PB Rocket', price: 10000, icon: '🚀' },
  { id: 'g6', name: '🐉 Fire Dragon', price: 50000, icon: '🐉' }
];

let activeRooms = [
  { id: 'room_101', name: '🔥 PB Underground Rap Battle', host: '☆ Lucky Ak47 🖥️☆', category: 'Rap', banner: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=400', seats: Array(12).fill(null) },
...
<div class="app">
  ...
  <div class="room" id="rScreen">
    ...
    <div style="height:50px; background:#10152e; display:flex; align-items:center; padding:0 10px; gap:8px;">
      ...
      <button class="btn" style="padding:6px 12px; border-radius:20px;" onclick="sendMsg()">➤</button>
    </div>
  </div>
...
<script>
    ...
    document.getElementById('rList').innerHTML = rms.map(r => \`
      <div class="card" onclick="openR('\${r.id}', '\${r.name}')">
        ...
        <div class="card-info">
          ...
          <div style="font-size:10px; color:#6a7bb2; margin-top:2px;">👑 \${r.host}</div>
        </div>
      </div>
    \`).join('');
  }
  ...
  function renderGiftGrid() {
    document.getElementById('gGrid').innerHTML = gifts.map(g => \`
      <div class="gift-item" id="gi_\${g.id}" onclick="selectGift('\${g.id}')">
        <div style="font-size:24px;">\${g.icon}</div>
        <div style="font-size:10px; font-weight:bold; margin-top:2px;">\${g.name}</div>
        <div style="font-size:9px; color:gold;">\${g.price} C</div>
      </div>
    \`).join('');
  }
  ...
  socket.on('rooms_updated', rms => { allRooms = rms; renderRooms(rms); });
  socket.on('room_created', r => openR(r.id, r.name));
  socket.on('gift_broadcast', d => {
    let b = document.getElementById('cBox');
    b.innerHTML += '<div style="color:gold; background:rgba(255,215,0,0.1); padding:4px; border-radius:6px; margin:2px 0;">🎁 <b>' + d.sender + '</b> ने <b>' + d.target + '</b> को ' + d.giftName + ' भेजा!</div>';
    b.scrollTop = b.scrollHeight;
  });
  socket.on('coin_updated', c => { user.coins = c; localStorage.setItem('pb_u', JSON.stringify(user)); syncMe(); });
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('PB Server running on ' + PORT);
});
