...
<script>
  ...
  function renderRooms(rms) {
    ...
    rms.forEach(r => {
      ...
      html += '<div style="font-size:10px; color:#6a7bb2; margin-top:2px;">👑 ' + r.host + '</div>';
      html += '</div></div>';
    });
  ...
  function drawStage() {
    ...
    seats.forEach((s, i) => {
      ...
      let innerText = s ? (s.isMuted ? '🔇' : s.name.substring(0,2)) : (i === 0 ? '👑' : (i + 1));
...
3. नीचे हरा बटन **`Commit changes`** दबाएँ।

---

अब Render पर 30 सेकंड बाद जब बिल्ड पास होकर **Live** हो जाएगा, तब किसी भी फोन से लिंक खोलकर चेक करें। सारे कंट्रोल्स, रूम क्रिएशन और गिफ्टिंग बॉक्स तुरंत काम करने लगेंगे।
