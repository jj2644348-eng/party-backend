...
let activeRooms = [
  { id: 'room_101', name: '🔥 PB Underground Rap Battle', host: '☆ Lucky Ak47 🖥️☆', banner: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=400', seats: Array(12).fill(null) },
  { id: 'room_102', name: '⚡ PB Cyber VIP Club', host: 'Mr Love', banner: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=400', seats: Array(12).fill(null) }
];
...
io.on('connection', (socket) => {
  socket.on('join_room', (d) => { if (d && d.roomId) socket.join(d.roomId); });
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: sans-serif; user-select: none; }
    ...
    .inp { background: #151a36; border: 1px solid #283366; border-radius: 8px; padding: 12px; color: #fff; outline: none; }
    ...
    .top-bar { height: 50px; padding: 0 14px; display: flex; justify-content: space-between; align-items: center; background: #10152e; border-bottom: 1px solid #1c244d; flex-shrink: 0; }
    .panel { flex: 1; display: none; flex-direction: column; overflow-y: auto; padding: 12px 12px 65px; }
    .panel.active { display: flex; }
    .grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
    .card { background: #121833; border-radius: 12px; overflow: hidden; border: 1px solid #1f2852; cursor: pointer; }
    .card img { width: 100%; height: 120px; object-fit: cover; }
    .bnav { height: 56px; background: #0c1022; border-top: 1px solid #1c244d; display: flex; justify-content: space-around; align-items: center; position: absolute; bottom: 0; left: 0; width: 100%; z-index: 100; }
    ...
    .room { position: absolute; inset:0; background: #070913; z-index: 200; display: none; flex-direction: column; }
    ...
    .ring.on { border-color: #00f0ff; color: #00f0ff; background: #1a2a4a; }
...
<div class="app">
  ...
  <div class="top-bar">
    ...
    <span style="background:gold; color:#000; padding:2px 8px; border-radius:10px; font-size:11px; font-weight:bold;" id="uCoins">0 C</span>
  </div>
  ...
  <div class="panel" id="tabMe">
    <div style="padding:20px; text-align:center;">
      ...
      <button class="btn" style="background:#1f2852;" onclick="logout()">Logout</button>
    </div>
  </div>

  <div class="room" id="rScreen">
    <div style="height:48px; display:flex; justify-content:space-between; align-items:center; padding:0 12px; background:#10152e;">
      <span id="rTitle" style="color:#00f0ff; font-weight:bold; font-size:12px;">Room</span>
      <div style="display:flex; gap:6px;">
        <button id="btnMic" class="btn" style="padding:4px 8px; font-size:10px; background:#1c244d; display:none;" onclick="toggleMic()">🎙️ Mic On</button>
  ...
  fetch('/api/rooms').then(r => r.json()).then(rms => {
    document.getElementById('rList').innerHTML = rms.map(r => \`
      <div class="card" onclick="openR('\${r.id}', '\${r.name}')">
        <img src="\${r.banner}">
        <div style="padding:8px; font-size:11px; font-weight:bold;">\${r.name}</div>
      </div>
    \`).join('');
  });
  ...
  function drawStage() {
    document.getElementById('stg').innerHTML = seats.map((s, i) => \`
      <div class="seat" onclick="toggleSeat(\${i})">
        ...
        <span style="font-size:8px; color:#6a7bb2; margin-top:2px;">\${s ? s.name : 'Mic '+(i+1)}</span>
      </div>
    \`).join('');
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('PB Server Running on port ' + PORT);
});
